...
__global__ void div_kernel(float *input, float *div_input, const unsigned int size) {
  const int id = blockDim.x * blockIdx.x + threadIdx.x;
  for (int i = id; i < size; i += blockDim.x * gridDimx.x) {
...
void forward_normalize_gpu(normalize_layer nl, float *input_gpu) {
  ...
  pow_kernel<<<opt_gridsize(nl.output_size, 512), 512>>>(input_gpu, nl.output_size, 2.0f);
  ...
  pow_kernel<<<opt_gridsize(nl.output_size, 512), 512>>>(nl.out_norm_gpu, nl.out_norm_size, 0.5f);
